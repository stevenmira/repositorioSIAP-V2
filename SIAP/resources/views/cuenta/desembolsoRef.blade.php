    ...
    <tbody>
      <tr>
        ...
        <td style="border: 1px solid #333; text-align: right;">
          ...
          <span class="pull-right" id="ppmonto">{{ number_format($desembolso, 2) }}</span></td>
...
{{Form::Open(['action'=>'CuentaController@desembolsoRefinanciamientoPDF', 'target'=>'_blank'])}}
<div class="padd">
  <table style="width: 100%; border-collapse: collapse;">
    <tr>
      <td style="width: 50%; font-weight:bold;">Desembolso</td>
      <td style="width: 25%; font-weight:bold;" id="ppdesembolso">$ {{ number_format($desembolso, 2)}}</td>
      <td>
        {!! Form::number('desembolso', $desembolso, ['placeholder'=>'Desembolso', 'step'=>'0.01', 'onkeyup'=>'suma()', 'id'=>'pdesembolso']) !!}
      </td>
      
    </tr>
    <tr>
      <td>( - Desc. De $4.50 de cada $100.00 por desembolso)</td>
      <td id="ppcomision">$ {{ number_format($comision, 2) }}</td>
      <td>
        {!! Form::number('comision', $comision, ['placeholder'=>'Comision', 'step'=>'0.01', 'onkeyup'=>'suma()', 'id'=>'pcomision']) !!}
      </td>
    </tr>
    <tr>
      @if($cuotas > 1)
      ...
      @endif
      <td id="pptotalCuota">$ {{ number_format($totalCuota, 2) }}</td>
      <td>
        {!! Form::number('totalCuota', $totalCuota, ['placeholder'=>'Total Cuota', 'step'=>'0.01', 'onkeyup'=>'suma()', 'id'=>'ptotalCuota']) !!}
      </td>
    </tr>
    <tr>
      <td>( - Desc. de mora por incumplimiento )</td>
      <td  id="ppmora">$ {{ number_format($mora, 2) }}</td>
      <td>
        {!! Form::number('mora', $mora, ['placeholder'=>'Mora', 'step'=>'0.01', 'onkeyup'=>'suma()', 'id'=>'pmora']) !!}
      </td>
    </tr>
    <tr>
      ...
      <td>
        <u>
          <span  id="ppsaldoCapitalAnterior">$ {{ number_format($saldoCapitalAnterior,2) }}</span>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        </u>
      </td>
      <td>
        {!! Form::number('saldoCapitalAnterior', $saldoCapitalAnterior, ['placeholder'=>'Saldo Capital Anterior', 'step'=>'0.01', 'onkeyup'=>'suma()', 'id'=>'psaldoCapitalAnterior']) !!}
      </td>
    </tr>
    <tr>
      <td>EFECTIVO A RECIBIR</td>
      <td style="font-weight: bold;" id="pptotal"><b>$ {{ number_format($total,2) }}</b></td>
    </tr>
  </table>
</div>
<br>
...
<input type="number" name="idcuenta" hidden="on" value="{{$cuenta->idcuenta}}">
<input type="number" name="idprestamo" hidden="on" value="{{$prestamo->idprestamo}}">
<input type="number" name="idcliente" hidden="on" value="{{$cliente->idcliente}}">
<input type="number" name="cuotas" hidden="on" value="{{$cuotas}}">
{{Form::Close()}}
