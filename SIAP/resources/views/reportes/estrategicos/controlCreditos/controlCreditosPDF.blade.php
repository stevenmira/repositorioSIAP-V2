			...
			<tr>
				<td>
					<p style="float: left;">
	 				...
	 				<p style="float: right;">
	      				Fecha Emisión: {{ \Carbon\Carbon::now()->format('d-m-Y') }}
	  				</p>
				</td>
			</tr>
	  	</table>
		 ...
		 <table  style="width: 100%; border-collapse: collapse; border: 1px solid #333;">
		    ...
		    <tbody>
              ...
              <tr style="font-size: 12px; font-weight: normal;border: 1px solid #333;">
                ...
                <td style="border: 1px solid #333; text-align: left;">{{$con->nombre}} {{$con->apellido}}</td>
                <td style="border: 1px solid #333; text-align: center;">{{$con->dui}}</td>
                <td style="border: 1px solid #333; text-align: center;">{{$con->fecha}}</td>
                <td style="border: 1px solid #333; text-align: right;"><span class="pull-left">&nbsp;$</span>{{ number_format($con->monto,2) }}</td>
                <td style="border: 1px solid #333; text-align: center;">{{ number_format($con->interes,2) }}%</td>
                <td style="border: 1px solid #333; text-align: right;"><span class="pull-left">&nbsp;$</span>{{ number_format($con->comision,2) }}</td>
                <td style="border: 1px solid #333; text-align: right;"><span class="pull-left">&nbsp;$</span>{{ number_format($con->montooriginal,2) }}</td>
                <td style="border: 1px solid #333; text-align: center;">{{$con->nombreCartera}}</td>

                @if($con->nombreDesembolso == 'EFECTIVO')
                <td style="border: 1px solid #333; text-align: center;">{{$con->nombreDesembolso}}</td>
                @else
                <td style="border: 1px solid #333; text-align: center;">{{$con->numerocheque}}</td>
                @endif
              </tr>
              @endforeach
